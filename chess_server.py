# -*- coding: utf-8 -*-
"""
Created on Mon Dec 12 07:18:30 2022
    ...
    if event.type==pygame.MOUSEBUTTONDOWN:
        ...
        for i in range(len(chess_board)):
            for j in range(len(chess_board[i])):
                if chess_board[i][j][0]<pos[0]<chess_board[i][j][0]+20 and chess_board[i][j][1]<pos[1]<chess_board[i][j][1]+20:
...
while True:
    ...
    for r in rs:
        if r is tcpsersock:
            ...
            if disconnected:
                inputs.remove(r)
                draw_text('your opponent is disconnected',200,420,15)
                while True:
